...
if __name__ == '__main__':
	option_url = ""
	option_xss = False
	option_crawler = False
	
	if len(sys.argv) > 1:
		parser = argparse.ArgumentParser()
		parser.add_argument("-u", "--url", dest="archives_url", help=" The URL Address to investigate")
		parser.add_argument("-x", "--xss", dest="xss", action="store_true",default=False, help="To perform XSS attacks")
		parser.add_argument("-d", "--crawler", dest="crawler", help="Look for every files in webpage")
	
		args = parser.parse_args()

		option_url = args.archives_url
		option_xss = args.xss
		if option_xss:
			print("You have chosen to test XSS attacks")
